...
export default {
	...
	setup() {
			...
			// 获取用户会话信息
			state.user = getSessionStorage('user');
	
			// 查询送货地址
			const listDeliveryAddressByUserId = () => {
				axios.get(`delivery-addresses/user/${state.user.userId}`)
					.then(response => {
						state.deliveryAddressArr = response.data;
					})
					.catch(error => {
						console.error(error);
					});
			...
			const setDeliveryAddress = (deliveryAddress) => {
				setLocalStorage(state.user.userId, deliveryAddress);
			};
	
			// 跳转到新增送货地址
			const toAddUserAddress = () => {
				router.push({ path: '/addUserAddress', query: { businessId: state.businessId } });
			};
	
			// 编辑送货地址
			const editUserAddress = (daId) => {
				router.push({ path: '/editUserAddress', query: { businessId: state.businessId, daId: daId } });
			};
			...
			const removeUserAddress = (daId) => {
				...
				axios.delete(`delivery-addresses/${daId}`)
					.then(response => {
						if (response.data > 0) {
							let deliveryAddress = getLocalStorage(state.user.userId);
							if (deliveryAddress && deliveryAddress.daId === daId) {
								removeLocalStorage(state.user.userId);
							}
